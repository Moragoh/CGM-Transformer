...
class Logger(object):
    def __init__(self, filename):
        self.terminal = sys.stdout; self.log = open(filename, "w")
...
def get_overall_performance(model_name, model_dir, test_loader):
    ...
    if is_student: model = CGMPredictor(n_embd=4*48, n_head=4, n_layer=2)
    else: model = CGMPredictor(n_embd=8*48, n_head=8, n_layer=3)
    ...
    try:
        ...
        print(f"--> ERROR loading {model_name}: {e}. Skipping."); return None, None
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            total_abs_error += torch.sum(torch.abs(error)).item(); total_squared_error += torch.sum(error**2).item(); total_samples += error.numel()
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=f"Analyzing {model_name}"):
            ...
            for i in range(1, num_intervals + 1):
                # --- THIS IS THE FIX ---
                # Convert pred_time to integer before comparing
                mask = (pred_time.long() == i)
                # --- END OF FIX ---
...
if __name__ == "__main__":
    ...
    print("\n" + "="*55); print("--- PART 1: IDENTIFYING THE BEST STUDENT MODEL ---"); print("="*55)
    ...
    for student_name in student_checkpoints:
        ...
        if mae is not None: student_performances[student_name] = mae
    ...
    print("\n" + "="*55); print("--- PART 2: DETAILED INTERVAL PERFORMANCE ANALYSIS ---"); print("="*55)
    ...
    print("\n" + "#"*75); print("--- FINAL REPORT: TEACHER VS. BEST STUDENT (PERFORMANCE BY INTERVAL) ---"); print("#"*75)
    print(f"Teacher Model: {TEACHER_BENCHMARK_FILE}"); print(f"Student Model: {best_student_name}\n" + "-"*75)
    header = f"{'Time (min)':<12} | {'Teacher MAE':<15} | {'Student MAE':<15} | {'Teacher RMSE':<15} | {'Student RMSE':<15}"; print(header); print("-"*75)
    
    pred_times_in_minutes = (1 + torch.arange(18)) * 5
    for i in range(18):
        time, t_mae, t_rmse, s_mae, s_rmse = pred_times_in_minutes[i].item(), teacher_mae[i].item(), teacher_rmse[i].item(), student_mae[i].item(), student_rmse[i].item()
