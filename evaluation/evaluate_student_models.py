...
BATCH_SIZE = 2 # Kept low to prevent OutOfMemory/Heat issues
MAX_LEN = 512 * 10 
...
if not student_checkpoints:
    print(f"No student model checkpoints found in '{MODEL_DIR}'.")
    sys.exit()
...
results = {}
criterion = nn.L1Loss()
for model_name in sorted(student_checkpoints, key=lambda x: int(x.split('_')[-1][:-4])):
    checkpoint_path = os.path.join(MODEL_DIR, model_name)
    print(f"\n--- Evaluating: {model_name} ---")

    model = CGMPredictor(n_embd=4*48, n_head=4, n_layer=2)

    # --- FIX: Added key cleaning logic for the compiled student models ---
    try:
        state_dict = torch.load(checkpoint_path, map_location=DEVICE)
        
        # Check if keys have the compile prefix and clean them if they do
        if any(k.startswith('_orig_mod.') for k in state_dict.keys()):
            print("-> Found compiled model prefix. Cleaning state_dict keys...")
            ...
            # Load directly if no prefix is found
...
# --- Final Report (Code is the same as before) ---
print("\n" + "="*40)
